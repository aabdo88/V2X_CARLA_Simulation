...
import os
import sys
project_folder = 'C:/Users/ahmed/OneDrive/Desktop/pythonProject'
try:
    sys.path.append(glob.glob(project_folder + 'carla/dist/carla-*%d.%d-%s.egg' % (
...
if __name__ == '__main__':
    ...
    while step < 300:
        ...
        for veh in vehicle_id_list:
            ...
            cv = Connect_Vehicle()
            cv.read_and_send_parameters(veh, draw_vehicle, 8)
            #cv.read_and_send_parameters(veh, draw_vehicle, 0)
            cvs.append(cv)
        pygame.display.update()
