...
class communications_service:
    ...
    def V2I_REQ(self, rsus, cvs):
        self.type = 'V2I'
        rsu_unit = pygame_rsu()
        for rsu in rsus:
            for cv in cvs:
                Inside_Range_status = rsu_unit.RSU_range_intersects(rsu, cv.pygame_car)
                if Inside_Range_status:
                    rsu_ID, data = control.send(rsu)
                    cv.receive_REQ(rsu_ID, data)
